...
def import_stops(db: Session, stops_df: pd.DataFrame):
    log.debug(f"Importing {len(stops_df)} stops...")
    ...
    log.debug(f"Found {len(parent_stops)} parent stops and {len(child_stops)} child stops.")
    upsert(db, Stop, parent_stops, ["id"], mapping)
    log.debug(f"Imported {len(parent_stops)} parent stops.")

    upsert(db, Stop, child_stops, ["id"], mapping)
    log.debug(f"Imported {len(child_stops)} child stops.")
