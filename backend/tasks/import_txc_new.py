...
class TXCImporter:
    ...
    def handle_journey(
        ...
        journey_code = self.get_id(txc_journey.ticket_machine_journey_code)
        ...
        if calendar_hash in self.calendar_cache:
            ...
            for so in txc_journey.operating_profile.serviced_organisations:
                for working_day in so:
                    cal_exep = self.get_calendar_exception(
        ...
        journey_pattern_section = self.txc_data.j[
    ...
    def handle_service(self, txc_service: txc.Service):
        ...
        for txc_line in txc_service.lines:
            ...
            if not db_line:
                ...
                print(f"Updating line {txc_line.id}")
                self.db.merge(line)
            self.db.flush()
            self.line_to_routes.setdefault(txc_line.id, []).extend(routes)

            if skip_journeys:
                continue
            for txc_journey in self.txc_data.get_journeys(
                txc_line.id, txc_service.service_code
            ):
                if not txc_journey.journey_code:
                    print(
                        f"Skipping journey for service {txc_service.service_code} on line {txc_line.id} due to missing journey code"
                    )
                    continue
                self.handle_journey(txc_journey, txc_service, service, line)

    def handle_route_section(self, txc_route_section: txc.RouteSection):
        track_points: list[txc.Point] = []
        track_points_to_create = []
        route_links = txc_route_section.links
        rs_id = self.get_id(txc_route_section.id)
        for link in route_links:
            ...
            trackpoint = TrackSection(
                ...
                # distance=link.track,
                geometry=from_shape(
                    LineString([[pt.longitude, pt.latitude] for pt in points])
                ),
                route_link_ref=self.get_id(link.id),
                route_section_id=rs_id,
            )
            track_points_to_create.append(trackpoint)

        if track_points:
            coords = [[pt.longitude, pt.latitude] for pt in track_points]
