...
async def get_scheduled(stop_id: str, redis):
    times = []
    ...
    with SessionLocal() as db:
        ...
        if use_db_method:
            ...
            if use_db_method:
                times = []
                for time, st in db_times:
                    journey = time.journey
                    trip_id = await journey.get_bt_trip_id(db)
                    if trip_id:
                        times.append({"trip_id": int(trip_id), "st": st})
...
async def get_departures(stop_id: str, redis):
    ...
    times, use_db_method = await get_scheduled(stop_id, redis)
